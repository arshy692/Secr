...
while True:
	try:
		...
		doc = types.InlineKeyboardButton("📃 Declaration", url=f'{bot_documentation}')
		shop = types.InlineKeyboardButton("🛍 Buy/shop", callback_data='shop')
		snoser = types.InlineKeyboardButton("🌐 Kill time", callback_data='snoser')
		menu.add(profile)
		menu.add(doc, shop)
		menu.add(snoser)

		back_markup = types.InlineKeyboardMarkup(row_width=2)
		back = types.InlineKeyboardButton("❌ cancel", callback_data='back')
		...
		add_subsribe = types.InlineKeyboardButton("Add subscribe", callback_data='add_subsribe')
		clear_subscribe = types.InlineKeyboardButton("Забрать подписку", callback_data='clear_subscribe')
		send_all = types.InlineKeyboardButton("Gcast", callback_data='send_all')
		...
		sub_2 = types.InlineKeyboardButton(f"🔹 7 day - {subscribe_7_days}$", callback_data='sub_2')
		sub_4 = types.InlineKeyboardButton(f"🔹 30 day- {subscribe_30_days}$", callback_data='sub_4')
		...
		def welcome(message):
			...
			bot.send_message(message.chat.id, f'♨️ *{bot_name}* — _инструмент для уничтожения аккаунтов._\n\n⚡️ *Админ: {bot_admin}*\n⭐️ *Отзывы:* [Reviews]({bot_reviews})\n🔥 *Работы:* [Works]({bot_works})', parse_mode="Markdown", reply_markup=menu)
			connect.close()
